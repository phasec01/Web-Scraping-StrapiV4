...
const main = async () => {
  ...
  try {
    ...
    const scraper = await strapi.db.query("api::scraper.scraper").findOne({
      slug: slug,
    });
    ...
    if (scraper == null || !scraper.enabled || !scraper.frequency) {
      console.log(
        `${"Exit"}: (Your scraper may does not exist, is not activated or does not have a frequency field filled in)`
      );
      return;
    }
